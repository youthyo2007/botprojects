...
public class MainDialog extends ComponentDialog {
    ...
    private MicrosoftTranslatorService translatorService;
     ...
     */
    public MainDialog(FlightBookingRecognizer withLuisRecognizer, DepartmentService departmentService, MicrosoftTranslatorService translatorService, BookingDialog bookingDialog) {
        super("MainDialog");
        ...
        return luisRecognizer.recognize(stepContext.getContext()).thenCompose(luisResult -> {
            switch (luisResult.getTopScoringIntent().intent) {
                ...
                case "Places_FindPlace":
                    ...
                    Activity geFindPlaceMessage = MessageFactory
                        .text(
                            FindPlacerMessageText, FindPlacerMessageText,
                            InputHints.IGNORING_INPUT
                        );


                        // Reply to the activity we received with an activity.
                        Activity reply = MessageFactory.attachment(Cards.getHeroDimarxioCard().toAttachment());
                        ...
                        .thenCompose(resourceResponse -> stepContext.getContext().sendActivity(
                            ...
                            MessageFactory.text("Please ask me anything else")
                        ))
                        .thenCompose(resourceResponse -> stepContext.endDialog());

                    /*return stepContext.getContext().sendActivity(geFindPlaceMessage)
                        .thenCompose(resourceResponse -> stepContext.next(null));
                        */


                case "Places_GetPhoneNumber":
                         // Extract the values for the composite entities from the LUIS result.
                        ...
                        return stepContext.getContext().sendActivity(geFindAddressMessage)
                            .thenCompose(resourceResponse -> stepContext.next(null));  

                case "Places_GetAddress":
                        // We haven't implemented the GetWeatherDialog so we just display a TODO message.
                         // Extract the values for the composite entities from the LUIS result.
                        ObjectNode objectAddressEntities = luisRecognizer.getAddressEntities(luisResult);
    
                        String getAddressMessageText = "The Address Of "+objectAddressEntities.get("Places_PlaceName").asText() +" is Koliatsou 32";
                        Activity getFindAddressMessage = MessageFactory
                            .text(
                                getAddressMessageText, getAddressMessageText,
                                InputHints.IGNORING_INPUT
                            );
                        return stepContext.getContext().sendActivity(getFindAddressMessage)
                            .thenCompose(resourceResponse -> stepContext.next(null));                        

                case "GetWeather":
                        // We haven't implemented the GetWeatherDialog so we just display a TODO message.
                        String getWeatherMessageText = "TODO: find Weather Flow  here";
